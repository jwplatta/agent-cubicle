...
This is a simple, local CLI to run my coding agents (Codex, Claude, Copilot, Gemini) while keeping all shared config, skills, commands, prompts, hooks, and other context in this repo (`cubicle`). This is intentionally non-production code: it should be simple, intelligible, and easy to change as my needs evolve.
...
- If a managed target path already exists and is not the expected symlink, move the existing path into a timestamped backup tree under `~/.cubicle/backups/` and then install the expected symlink.
...
Create a script in the repo root named `cubicle`. We will run it from this repo. It should support:

- `cubicle run --agent <codex|claude|copilot|gemini> --project <name>`
- `cubicle clean --agent <codex|claude|copilot|gemini>`
- `cubicle help` (or `-h/--help`) with a brief description
